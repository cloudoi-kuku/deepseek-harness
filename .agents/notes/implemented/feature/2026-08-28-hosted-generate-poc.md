...
Envon Container App `ca-envon-generate-poc` (`packages/experimental/hosted-generate/example/azure/Dockerfile.web`) serves the published `dsh web` UI behind that proxy and the generate HTTP contract on `/generate` and `/sessions/`. Generate runs `dsh --profile headless` with filesystem tools only (`generate.patch.yml`). Grok is the default model via `llm-bridge.mjs`: Foundry deployment `grok-4-3` when `FOUNDRY_API_KEY` is set (CAE VNet private PE), otherwise `https://api.x.ai/v1` with `XAI_API_KEY`. Scale remains `minReplicas=0` / `maxReplicas=1`. The keyless generate mock (`Dockerfile` + `server.mjs`) is unchanged. CoreNet overlays the returned file map onto its scaffold and remains the GitHub writer.
