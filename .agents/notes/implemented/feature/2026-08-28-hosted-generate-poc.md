...
**Install published `@deepseek-ai/dsh` from npm in `Dockerfile.web`.** Rejected for the Envon web image: that tag does not include `workspace-source-git` or `workspace.createGit`. The image builds `@deepseek-ai/dsh` from this repository (`pnpm --filter @deepseek-ai/dsh deploy`) so those packages exist, then copies the CoreNet overlay.

**Mount `principal-hmac` in the live Envon composition.** Rejected for this image: operator `GENERATE_TOKEN` has no launch cookie, and an authenticator would empty `listVisible`. Overlay HMAC in `launch-token.mjs` plus `workspace.create({ path })` remain the live pin. Hosted principal is a later patch.
...
Envon Container App `ca-envon-generate-poc` (`examples/hosted-generate/azure/Dockerfile.web`, repository-root context) serves source-built `dsh web` behind that proxy and the generate HTTP contract on `/generate` and `/sessions/`. `workspace-github.mjs` clones the CoreNet GitHub grant into `/workspace` and pins it with `workspace.create({ path })`. `generate-server.mjs` also serves `/corenet/publish` and `/corenet/database` (`corenet-bridge.mjs`). Generate runs `dsh --profile headless` with filesystem tools only (`generate.patch.yml`). Grok is the default model via `llm-bridge.mjs`: Foundry deployment `grok-4-3` when `FOUNDRY_API_KEY` is set (CAE VNet private PE), otherwise `https://api.x.ai/v1` with `XAI_API_KEY`. Default `--trusted-host` is `harness.cloudoi.io`. The live composition does not mount `principal-hmac`. Scale remains `minReplicas=0` / `maxReplicas=1`. The keyless generate mock (`Dockerfile` + `server.mjs`) is unchanged. CoreNet remains the GitHub writer for generate artifacts; the web overlay also clones the user's grant into cwd.
