...
Envon Container App `ca-envon-generate-poc` (`packages/experimental/hosted-generate/example/azure/Dockerfile.web`) serves published `dsh web` at `/` behind that proxy. A CoreNet launch token at `/?launch=` mounts GitHub into `/workspace` ([GitHub cwd](2026-08-29-envon-github-cwd.md)). `/new` redirects to `/`. `POST /generate` and `/sessions/` remain the headless file-map path for the CoreNet wizard (`generate.patch.yml`, filesystem tools only). Grok is the default model via `llm-bridge.mjs`: Foundry deployment `grok-4-3` when `FOUNDRY_API_KEY` is set (CAE VNet private PE), otherwise `https://api.x.ai/v1` with `XAI_API_KEY`. Scale remains `minReplicas=0` / `maxReplicas=1`. The keyless generate mock (`Dockerfile` + `server.mjs`) is unchanged. CoreNet remains the writer of GitHub (workspace grant and wizard publish) and `{slug}.cloudoi.dev`.
