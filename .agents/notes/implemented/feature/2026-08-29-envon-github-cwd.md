...
`HarnessWorkspaceTests` covers launch URL `/?launch=` (not `/new`), create-on-404, reuse-without-seed, and missing GitHub token. `examples/hosted-generate/tests/workspace-github.spec.ts` covers grant JSON parse, token-free clone URL, Bearer fetch, stderr redaction, the dsh `workspace.create` RPC envelope, rename to `owner/name`, retry while dsh is down, and fail-loud business RPC errors. `examples/hosted-generate/tests/launch-token.spec.ts` covers HMAC claims, reuse until expiry, and rejection of a token that claims longer than ten minutes.
