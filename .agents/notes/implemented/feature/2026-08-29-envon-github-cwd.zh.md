...
`HarnessWorkspaceTests` 覆盖 launch URL 为 `/?launch=`（不是 `/new`）、404 时创建、复用且不补种、以及缺少 GitHub token。`examples/hosted-generate/tests/workspace-github.spec.ts` 覆盖 grant JSON 解析、不含 token 的 clone URL、Bearer 拉取、stderr 脱敏、dsh `workspace.create` RPC 信封、重命名为 `owner/name`、dsh 未就绪时的重试，以及业务 RPC 错误时立即失败。`examples/hosted-generate/tests/launch-token.spec.ts` 覆盖 HMAC 声明、过期前复用，以及声称超过十分钟的 token 被拒绝。
