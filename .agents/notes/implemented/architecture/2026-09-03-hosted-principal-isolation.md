...
`ctx.principal` (`dsh-principal`) is an optional Service Definition: authenticators register by id, `bindFromRequest` identifies, and `run` binds the caller on `AsyncLocalStorage` for one Host request. Default `dsh web` does not mount it. The Host HTTP carrier (`dsh-client-connection`) calls `bindFromRequest` and `run` around each RPC Fetch, and copies authenticator `Set-Cookie` on `auth.logout`; Typert Remotes do not see the Request. The Envon overlay pins via Typert `workspace/create` over loopback; `web-rpc.patch.yml` sets `requireBrowserSession: false` on dsh web so the pin and the index can run without the printed `?token=` URL (dsh still binds 127.0.0.1 only; the public proxy accepts CoreNet HMAC or GENERATE_TOKEN). Headless generate does not apply that patch. `dsh-principal-hmac` re-validates the CoreNet launch token (cookie `harness_launch` or `Authorization: Bearer`) inside dsh; the overlay proxy keeps its own HMAC in `packages/experimental/hosted-generate/example/azure/launch-token.mjs` and is not rewritten. `auth.me` / `auth.logout` are Workspace-controller Host Remotes (`auth` namespace). Logout emits `Set-Cookie` Max-Age=0; expiry is `exp`; there is no revocation list in this slice.

When authenticators are registered, workspace records stamp optional `owner: { tenantId, userId }` (domain version stays 3 so existing Envon units keep loading). Host RPC uses `listVisible` / `getVisible`: another tenant's id looks like not-found. OSS (no authenticators) still lists every record and `create({ path })` is unchanged.
...
`workspace.gitStatus` / `gitCommit` / `gitPush` / `gitPull` / `gitCheckoutBranch` are Workspace Remote methods over the existing git provider. `dsh-hosted-limits` supplies `killSwitch`, per-user workspace and live-session caps (`0` = unlimited), a process-local git-op rate limit, and `checkoutRoot`. Unmounted compositions have no quotas.
...
Hosted patches that mount `principal` + `principal-hmac` + `hosted-limits` isolate git checkouts and filter workspace RPC by owner. Compositions that do not mount them keep local-path behavior, including the CoreNet `create({ path })` pin. Optional `owner` on workspace records is backward-compatible with domain version 3. GitHub OAuth, server-side revocation, container isolation, dedicated audit/grants, and IDE surfaces remain later work.
