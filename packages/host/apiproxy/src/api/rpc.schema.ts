...
export const rpcErrorSchema: z.ZodType<RpcError> = z.discriminatedUnion('code', [
  ...
  z.object({ code: z.literal('workspace-invalid-remote'), message: z.string(), details: z.object({ remoteUrl: z.string() }) }),
  z.object({ code: z.literal('workspace-source-unavailable'), message: z.string(), details: z.object({ kind: z.string() }) }),
  z.object({
    code: z.literal('workspace-prepare-failed'),
    message: z.string(),
    details: z.object({ remoteUrl: z.string().optional(), path: z.string().optional() }),
  }),
  ...
  z.object({ code: z.literal('forbidden'), message: z.string(), details: z.object({ workspaceId: z.string().optional() }) }),
  z.object({ code: z.literal('quota-exceeded'), message: z.string(), details: z.object({ kind: z.string(), limit: z.number() }) }),
  z.object({ code: z.literal('rate-limited'), message: z.string(), details: z.object({ kind: z.string(), limit: z.number() }) }),
  z.object({ code: z.literal('kill-switch'), message: z.string(), details: z.object({}) }),
  z.object({ code: z.literal('workspace-not-git'), message: z.string(), details: z.object({ workspaceId: z.string() }) }),
  z.object({
    code: z.literal('workspace-checkout-unresolved'),
    message: z.string(),
    details: z.object({ reason: z.union([z.literal('parent-required'), z.literal('root-missing')]) }),
  }),
