...
- `ctx.workspaceRegistry.create(path, title?)` — canonicalizes `path` via `fs.realpath`, rejects a nonexistent or non-directory path, writes `{ kind: 'local', path }` as `source`, creates at most one record per canonical path, and prepends a new record to durable workspace order. Repeated calls for that path return the existing workspace without changing its title; different paths may share a display title. Hosted overlays that clone first and then call `workspace.create({ path })` keep this path.
- `ctx.workspaceRegistry.createGit({ remoteUrl, checkoutParent?, branch?, owner?, repo?, credentialId?, title? })` — requires `ctx.workspaceSource`. Resolves the remote, prepares the checkout (clone or fetch), and writes a git `source` that never includes a token. Repeated calls for the same canonical checkout path return the existing workspace without changing its title. When `ctx.principal` has authenticators, `checkoutParent` is ignored and the parent is `hostedLimits.checkoutRoot/<tenantId>/<userId>`; without authenticators, `checkoutParent` is required. New records stamp `owner` from the bound principal.
- `ctx.workspaceRegistry.listVisible()` / `getVisible(id)` — same as `list`/`get` when no authenticators are registered; otherwise only records whose `owner` matches the bound principal (empty / undefined when none is bound).
...
- `createGit` stores an optional `credentialId` but does not look it up; clone uses the process Git environment.
- File-tree, editor tabs, diff, terminal, preview, search, and SCM UI are later IDE surfaces ([note](../../../.agents/notes/implemented/architecture/2026-09-03-hosted-principal-isolation.md)).
- Dedicated durable audit/grant tables are not in this package; owner is stored on the workspace record, session logs and settings stay on their existing seams, and git checkouts remain disposable.
