...
Live app `ca-envon-generate-poc` on `cae-envon-prd-eus2-01` serves **`dsh web` at `/`** and the generate HTTP contract at `https://harness.cloudoi.io`. Image: `packages/experimental/hosted-generate/example/azure/Dockerfile.web`, built from the repository root so `dsh` includes `workspace-source-git`. `dsh web` binds `127.0.0.1:3080` with cwd `/workspace`; `generate-server.mjs` binds `127.0.0.1:3081` and runs `dsh --profile headless` with `grok.patch.yml` plus `generate.patch.yml` (filesystem tools only). `web-proxy.mjs` binds `0.0.0.0:8080`, requires a CoreNet launch token or `GENERATE_TOKEN` as HTTP Basic password / Bearer, and routes `/generate` and `/sessions/` to the generate server. A valid `/?launch=` token fetches `GET /api/ai-build/from-harness/workspace`, clones that GitHub repo into `/workspace`, POSTs dsh `workspace.create` for that directory (title `owner/name`) so the composer is live without the directory picker, and 302s to `/`. `/new` redirects to `/`. `--trusted-host` is `harness.cloudoi.io`. The live composition does not mount `principal-hmac`. Scale: `minReplicas=0` / `maxReplicas=1` at 1.0 vCPU / 2 Gi.
...
`Dockerfile` + `server.mjs` remain the keyless generate-contract mock (no dsh, no model tokens). The Envon harness is a CloudOI surface: a CoreNet launch token (`GET /api/ai-build/harness-launch` → `/?launch=`) binds the session to that account and mounts GitHub as cwd. Wizard `POST /generate` still calls `POST /api/ai-build/from-harness/launch` so CoreNet writes a site repo and `{slug}.cloudoi.dev`. Operator `GENERATE_TOKEN` remains for rehearsal on an empty `/workspace`. Isolation is still not multi-tenant.
